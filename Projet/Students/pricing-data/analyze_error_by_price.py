...
def plot_error_analysis(df, results, output_prefix='pricing-data/results/error_analysis'):
    ...
    ax.set_ylim(0, min(df['relative_error'].quantile(0.95), 100))
    ax.grid(True, alpha=0.3)
    ...
    print(f"\nOK: Graphique sauvegardé: {output_prefix}.png")
    ...
    print(f"OK: Graphique binné sauvegardé: {output_prefix}_binned.png")
...
def main():
    ...
    for exp_name, validation_file in experiments:
        if not Path(validation_file).exists():
            print(f"\nWarning:  Fichier manquant: {validation_file}")
            continue
    ...
    print(f"\nOK: Résultats sauvegardés: {output_csv}")
